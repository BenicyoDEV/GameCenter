//ainda não sendo utilizado.


// ---------- dados (localStorage) ----------
function lerDados(chave) { return JSON.parse(localStorage.getItem(chave)) || []; }
function salvarDados(chave, lista) { localStorage.setItem(chave, JSON.stringify(lista)); }
 ...
 function mostrarSenha() {
  ...
  formLogar.addEventListener('submit', function(event){
    ...
    try {
      ...
      if (inUsuario !== usuarioCadastrado.usuario || inSenha !== usuarioCadastrado.senha)
      {
...
function iniciarMenuPrincipal() { 
  const torneios = document.getElementById("torneio");
      ...
      const criarTorneio = document.getElementById('primeiroCriarTorneio');
...
function iniciarGerenciarJogadores() {
...
// ---------- roda só a tela que está aberta ----------
const pagina = document.body.dataset.pagina;
if (pagina === 'login') iniciarLogin();
if (pagina === 'cadastro') iniciarCadastro();
if (pagina === 'menuprincipal') iniciarMenuPrincipal();
if (pagina === 'gerenciarjogadores') iniciarGerenciarJogadores();
